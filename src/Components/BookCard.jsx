...
const BookCard = ({ book, token, isExpanded, onExpand, onRemove }) => {
    ...
    const checkIfFavorite = async () => {
      ...
      try {
        const response = await axios.get(
          "https://book-store-backend-0839.onrender.com/wishlist",
          {
            headers: {
  ...
  const handleBuyNowClick = async () => {
    ...
    try {
      const response = await axios.get(
        `https://book-store-backend-0839.onrender.com/user/address/${userEmail}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleBuyConfirm = async () => {
    try {
      ...
      const response = await axios.post(
        "https://book-store-backend-0839.onrender.com/purchase",
        {
